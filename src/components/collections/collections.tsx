...
export const Collections = ({
  ...
  const containerClass =
    isHorizontal || isLoading
      ? 'flex overflow-x-auto hide-scrollbar space-x-6'
      : 'grid grid-cols-2 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6';
    ...
    <>
      ...
      <ul role="list" className={containerClass}>
        {isLoading
          ? ['1a', '2a', '3a', '4a', '5a', '6a'].map(i => (
              <li
                ...
                className={classNames(
                  'flex justify-center relative top-0',
                  isHorizontal ? 'w-96' : ''
                )}
              >
                <CardSkeleton />
          ...
          : collections &&
            collections?.map(
              ({ isFeatured, image, name, slug, ownersWithTwitterCount }) => (
                <CollectionsItem
                  key={slug}
                  description={
                    ownersWithTwitterCount
                      ? `${ownersWithTwitterCount} members on 𝕏`
                      : undefined
                  }
