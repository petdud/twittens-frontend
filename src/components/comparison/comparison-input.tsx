...
interface IComparisonInputProps {
  id: string;
  addCollection: (collections: string) => void;
  removeCollection: (collections: string) => void;
}

export const ComparisonInput = ({id, addCollection, removeCollection}: IComparisonInputProps) => {
  const { data: collections } = useCollections({status: "active", select: "name,slug,image.thumbnailUrl"});
