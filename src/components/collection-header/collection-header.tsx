...
const SELECT_FROM_COLLECTION =
  'address,description,discordUrl,externalUrl,image,name,numberOfOwners,ownersWithTwitterCount,totalSupply,twitterUsername';
...
export const CollectionHeader = ({ slug }: ICollectionHeader) => {
  ...
  if (!data && !isLoading) {
    return (
      <div className="py-6">
        <MainViewHeader
          title={<div>Sorry, we didn&apos;t find a collection called {slug} 🫣.</div>}
        />
      </div>
    );
  }
        ...
        <div className="lg:pt-1 pt-0 flex flex-col lg:flex-row justify-between grow">
          <div className="max-w-xs sm:max-w-sm xl:max-w-xl overflow-hidden flex gap-1 flex-col">
            ...
            <CollectionInfo
              ...
              supply={totalSupply}
              twitterAccountsCount={ownersWithTwitterCount}
            />
          </div>
...
const BreadCrumbNavigation = () => {
  ...
  const onNavigationClick = () => (canGoBack ? router.back() : router.push('/'));
        ...
        <ol role="list" className="flex items-center space-x-4">
          <li className="text-sm">
            <button
              ...
              aria-current="page"
              className="font-medium text-gray-500 dark:text-neutral-300 hover:text-gray-600 dark:hover:text-white cursor-pointer hover:underline"
            >
...
const CollectionTitle = ({ name }: ICollectionTitleProps) => (
  <h1 className="inline-block w-full text-2xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-neutral-200">
    {name}
    <span>
      <span className="font-normal">&nbsp;on</span> 𝕏!
    </span>
  </h1>
);
...
const CollectionDescription = ({ description }: { description: string }) => {
  ...
  return (
    ...
    >
      ...
      >
        {seeMore ? (
          <ReactMarkdown className="whitespace-normal">{description}</ReactMarkdown>
        ) : (
          description
        )}
      </div>
  ...
  <div className="text-sm font-medium text-gray-500 dark:text-neutral-400 pt-1 lg:pt-4">
    <div className="flex gap-3">
      {supply && (
        <div>
          Items:{' '}
          <span className="dark:text-slate-200 text-neutral-700 font-semibold">
            {supply}
          </span>
        </div>
      )}
