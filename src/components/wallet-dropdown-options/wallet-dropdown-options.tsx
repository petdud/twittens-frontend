...
const marketplaceProviders = [
  ...
  //   getLink: (address: string, _slug: string) =>
  //     !address.includes(':') && `https://looksrare.org/collections/${address}`
  // },
  // {
  //   name: 'Rarible',
  //   logo: `${PREFIX_IMAGE_PATH}/rarible.png`,
  //   getLink: (address: string, _slug: string) =>
  //     !address.includes(':') && `https://rarible.com/collection/${address}`
  // },
  // {
  //   name: 'Coinbase',
  //   logo: `${PREFIX_IMAGE_PATH}/coinbase.png`,
  //   getLink: (address: string, _slug: string) =>
  //     !address.includes(':') && `https://nft.coinbase.com/collection/ethereum/${address}`
  // },
  // {
  //   name: "Sudoswap",
  ...
  // },
  {
    ...
    getLink: (address: string, _slug: string) =>
      !address.includes(':') && `https://blur.io/collection/${address}`
  }
  // {
  //   name: "Gem",
  ...
  //   getLink: (_address: string, slug: string) =>
  //     `https://dappradar.com/hub/nft-collection/${slug}`
  // },
  // {
  //   name: 'Zerion',
  //   logo: `${PREFIX_IMAGE_PATH}/zerion.png`,
  //   getLink: (_address: string, slug: string) =>
  //     `https://app.zerion.io/collections/${slug}`
  // }
];
...
export const WalletDropdownOptions = ({
  address,
  name,
  appearance
}: IWalletDropdownOptionsProps) => (
  <Dropdown
    name={<div className="flex items-center gap-2">{name}</div>}
...
export const MarketplaceDropdownOptions = ({
  appearance,
  address,
  name,
  slug,
  icon
}: IMarketplaceDropdownOptionsProps) => (
  <Dropdown
    name={!icon && <div className="flex items-center gap-2">{name}</div>}
    icon={icon}
    appearance={appearance}
  >
